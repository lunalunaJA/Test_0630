  		...
  		, success : function(data){
  			if(data.status == "0000") {
  				...
  				for(var i=0; i<data.result.length; i++) {
  					...
  					createNode("#znTreeDept",tree_parent_code, treeId, treeText, "last", POPTREE["DEPT"], treeCode, treeText, "N", "Y",true);
  				}
				
  				var tObj = $("#znTreeDept").jstree(true).get_node(deptid);
 		...
 		, success : function(data){
 			if(data.status == "0000") {
 				...
 				if(treeSelectedId_Class == "DEPT"){
 					...
 					for(var i=0; i<data.result.length; i++) {
 							AreaUserData_dataHtml += "<tr><td><input type=checkbox name="+keyName+" id="+keyName+i+" value='"+data.result[i].deptuserid+"' textData='"+data.result[i].zappUser.name+"' ><label for="+keyName+i+"><label></td><td>"+data.result[i].zappUser.name+"("+data.result[i].zappUser.empno+")</td></tr>";
 					}
 				}else {
 					...
 					for(var i=0; i<data.result.zappGroupUserExtends.length; i++) {
 						if(!objectIsEmpty(data.result.zappGroupUserExtends[i].zappUser.name)){
 							...
 							AreaUserData_dataHtml +="<input type=checkbox name="+keyName+" id="+keyName+i+" value='"+data.result.zappGroupUserExtends[i].groupuserid+"' textData='"+data.result.zappGroupUserExtends[i].zappUser.name+"' disabled>";
 							AreaUserData_dataHtml +="</td>";
 							...
 							AreaUserData_dataHtml += "<tr>";
 							AreaUserData_dataHtml += "<td>";
 							AreaUserData_dataHtml +="<input type=checkbox name="+keyName+" id="+keyName+i+" value='"+data.result.zappGroupUserExtends[i].groupuserid+"' textData='"+data.result.zappGroupUserExtends[i].zappDept.name+"' disabled>";
 							AreaUserData_dataHtml +="</td>";
	...
	console.log("GroupUserReg");
  	...
  	var result = $("#znTreeDept").jstree('get_checked');// get_checked get_selected
  	   ...
  	   for(var i=0; i<result.length;i++){
  			...
  			if(GroupUserDupCheck(node.id) != "DUP" ){//중복체크
  				GroupUserData_dataHtml += "<tr><td><input type=checkbox name=GroupUser id=GroupUser value='"+node.id+"' valueType='02' dataType='temp' ></td><td>"+node.text+"</td><td><spring:eval expression="@${lang}['DEPARTMENT']"/></td></tr>";	
  			}
    	   }//end for
  	...
  	$("input[name=OrgnAreaUser]:checked").each(function(idx) {
  		...
  		if(GroupUserDupCheck($(this).val()) != "DUP"){//중복체크
  			GroupUserData_dataHtml = "<tr ><td><input type=checkbox name=GroupUser id=GroupUser value='"+$(this).val()+"' valueType='01' dataType='temp' ></td><td>"+$(this).attr("textData")+"</td><td><spring:eval expression="@${lang}['USER']"/></td></tr>";
  			$("#GroupUserData").append(GroupUserData_dataHtml);
  		}
  	});
  	...
  	$("input[name=SearchAreaUser]:checked").each(function(idx) {
  		...
  		if(GroupUserDupCheck($(this).val()) != "DUP"){//중복체크
  			...
  			GroupUserData_dataHtml = "<tr ><td><input type=checkbox name=GroupUser id=GroupUser value='"+$(this).val()+"' valueType='"+$(this).attr("valueType")+"' dataType='temp' ></td><td>"+$(this).attr("textData")+"</td><td>"+sc_dataText+"</td></tr>";
  			$("#GroupUserData").append(GroupUserData_dataHtml);
  		}
  	});
  };
...
//그룹정보 검색
  				...
  				success : function(data){
  					...
  					if(data.status == "0000"){
  						...
  						for(var i=0; i<data.result.length; i++) {
  							...
  							SearchAreaUserData_dataHtml += "<td><input type='checkbox' name='SearchAreaUser' id='SearchAreaUser"+i+"' value='"+rs_deptuserid+"' textData='"+rs_name+"' valueType='01' ><label for='SearchAreaUser"+i+"'></label></td>";
  							SearchAreaUserData_dataHtml += "<td>"+rs_name+"</td></tr>";
  						}
  				...
  				success : function(data){
  					if(data.status == "0000"){
  						...
  						for(var i=0; i<data.result.length; i++) {
  							...
  							SearchAreaUserData_dataHtml += "<tr ><td><input type='checkbox' name='SearchAreaUser' id='SearchAreaUser"+i+"' value='"+rs_deptid+"' textData='"+rs_name+"' valueType='02' ><label for='SearchAreaUser"+i+"'></label></td><td>"+rs_name+"</td></tr>";
  						}
  			...
  			if(data.status == "0000") {
  				...
  				for(var i=0; i<data.result.length; i++) {
  					...
  					if(scItemRootType == "99"){
  							createNode("#", treeId, treeText, "last", "${image}/jstree/ic_tree_nomal_01.png", treeCode, treeText, "N", isactive, scItemRootType,priority);
  					}else{
  					  	var icon = (isactive == "Y") ? "${image}/jstree/ic_tree_nomal_01.png": "${image}/icon/img_folder_off.png";
  							createNode("#"+scItemRootType, treeId, treeText, "last",icon , treeCode, treeText, "N", isactive, scItemRootType,priority);
  					}
  			...
  			var GroupUserData_dataHtml = "";
  				...
  				if(data.status == "0000") {
  					...
  					for(var i=0; i<data.result.zappGroupUserExtends.length; i++) {
  						...
  						GroupUserData_dataHtml += "<tr><td><input type=checkbox name=GroupUser id=GroupUser value='"+groupid+"' valueGroupuserid='"+groupuserid+"' valueType='"+gobjtype+"' dataType='real' "+((editable == "N")?"disabled":"enabled")+"></td><td>"+groupname+"</td><td>"+gobjtypeStr+"</td></tr>";
  					}
  				}
  ...
  var initTreeData =function(type,name,icon){
  	return {
  		...
  		,"li_attr": {},
  		"a_attr": {"itemcode":type,"itemname":name,"itemtype":"N","itemisactive":"Y","itemroottype":type,class:"no_checkbox"} 
  	}
  }
//트리 초기화(부서)
  var initGroupTree = function(){
  	var initData =[];
  	initData.push(initTreeData(GROUPCODES["COMPANY"],"<spring:eval expression="@${lang}['COMPANY_GROUP']"/>",POPTREE["COMPANY"]));
  	initData.push(initTreeData(GROUPCODES["DEPT"],"<spring:eval expression="@${lang}['DEPARTMENT_GROUP']"/>",POPTREE["DEPTGROUP"]));
  	$('#znTreeGroup').jstree({
  		"checkbox" : {
  			three_state : false,
  			whole_node :  false,
...
//그룹 조회
  ...
  */
  var renderDeptGroupList = function(aclobjid, className, classType, classAcls, contentAcls){
  	...
  	var selectClass ={
  			select :2, // 1:조회 O, 등록 X, 2:조회 O, 등록 O
  			start:1,
  			end : classAcls.length
  	} 
  	var contentClass ={
  			select :contentAcls.length-1, //편집
  			start:0,
  			end : contentAcls.length
  	}
  	...
  	$innerHtml += renderAcls(classAcls, selectClass.select, selectClass.start, selectClass.end)
  	$innerHtml += "</select></td>";
  	$innerHtml += "<td><select id='InfoContentAcls'  style='width:100%'>";
  	$innerHtml += renderAcls(contentAcls, contentClass.select, contentClass.start, contentClass.end);
  	$innerHtml += "</select></td>";
...
//문서함정보 삭제
  ...
  var NodeReg = function(){
  	...
  	if( $.trim($("#ClassId").val()) == ""){
  	    alert("<spring:eval expression="@${msgLang}['ENTER_FOLDER_CODE']"/>");
  	    $("#ClassId").focus();
  	    return;
  	}
  	...
  	console.log("forderReg data : ", JSON.stringify(sendData));
  	
  	 noty({
		    layout : "center",
		    text : "<spring:eval expression="@${msgLang}['DO_YOU_FOLDER_INFO_REGISTERED']"/>",
		    buttons : [ {
		      addClass : 'b_btn',
		      text : "Ok",
		      onClick : function($noty) {
		        ...
		        $.ajax({
		    			...
		    			success : function(data){
		    				//console.log("data : "+JSON.stringify(data));
		    				if(data.status == "0000"){
		    					alert("<spring:eval expression="@${msgLang}['REGISTERED_FOLDER_INFO']"/>");
		    					folderRegClose();
				...
				<div>
					...
					<div>
						<div style="margin:0px; margin-top: 30px;">
							<ul class="tabmenu2">
								<li class="on" id="tab_Btn_Organ"><spring:eval expression="@${lang}['ORGANIZATION']" /></a></li>
								<li id ="tab_Btn_Group"><spring:eval expression="@${lang}['GROUP']" /></a></li>
								<li id ="tab_Btn_Search"><spring:eval expression="@${lang}['SEARCH']" /></a></li>
							</ul>
									...
									<div>
										...
										<div class="info_tb" style="margin-top: 5px; height: 140px; overflow-y: auto;">
											<table class="pop_tbl">
												...
												<thead>
													<tr>
														<th><input type=checkbox name="SearchAreaUserALLCheck"
															id="SearchAreaUserALLCheck" keydown /></th>
														<th><spring:eval expression="@${lang}['AUTHORITY_TARGET']" /></th>
													</tr>
												</thead>
