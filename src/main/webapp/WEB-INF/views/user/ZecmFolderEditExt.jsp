...
<script type="text/javascript">
		...
		$('#znTreeDept').jstree({
			...
			'core' : {
				'data' : [ {
					"id" : companyid,
					"text" : companyName,
					"icon" : "${image}/jstree/ic_enterprise_01.png",
					"state" : {
						"opened" : true,
						"disabled" : false
					},
				...
				if (data.status == "0000") {
					...
					for (var i = 0; i < data.result.length; i++) {
						...
						createNode("#znTreeDept", tree_parent_code, treeId, treeText, "last",
								"${image}/jstree/ic_department_04.png", treeCode, treeText, "N", "Y", true);
				...
				success : function(data) {
					if (data.status == "0000") {
						...
						for (var i = 0; i < data.result.length; i++) {
							...
							SearchAreaUserData_dataHtml += "<tr ><td><input type=checkbox name=SearchAreaUser id='SearchAreaUser"+i+"' value='"+rs_deptuserid+"' textData='"+rs_name+"' valueType='01' ><label for='SearchAreaUser"+i+"'></label></td><td>"
									+ rs_name + "</td></tr>";
						}
					} else {
	...
	var initGroupTree = function() {
		...
		var initData = [];
		initData.push(initTreeData("01", "<spring:eval expression="@${lang}['COMPANY_FOLDER_BOX']"/>", TREEICONS["COMPANY"]));
		initData.push(initTreeData("02", "<spring:eval expression="@${lang}['DEPARTMENT_GROUP']"/>", TREEICONS["DEPTGROUP"]));
		initData.push(initTreeData("04", "<spring:eval expression="@${lang}['COLLABORATIVE_GROUP']"/>", TREEICONS["COLLABOGROUP"]));
		$('#znTreeGroup').jstree({
			"checkbox" : {
				//"keep_selected_style" : false
				three_state : false,
				...
				if (data.status == "0000") {
					...
					for (var i = 0; i < data.result.length; i++) {
						...
						if (type == "98") { //전체사용자그룹 최상위 루트가 없어서 #으로 최상위를 지정해준다
							createNode("#znTreeGroup", "#", treeId, treeText, "last",
									"${image}/jstree/tree_user_icon08.png", treeCode, treeText, "N1", "Y", false);
						} else {
							createNode("#znTreeGroup", "#" + type, treeId, treeText, "last",
									"${image}/jstree/tree_user_icon08.png", treeCode, treeText, "N1", "Y", false);
						}
					}
				}
...
<!-- 팝업 -->
	<div class="popup" style="display: block; width:97%; height:89%; top:5px; border:0px; box-shadow:none; padding:15px;">
		<h3 class="pageTit" style="font-size:20px; margin-bottom:25px;">폴더 정보</h3>
		<ul class="tabmenu">
			<li class="on">기본 정보</li>
			<li>권한 설정</li>
		</ul>
		<div class="tabCont" style="height:430px;">
			...
			<div class="contdiv" id = "cont02_1">
				<div>
					...
					<div style='overflow: auto; margin:0px; height: 134px; width:100%;'>
						<table class="pop_tbl">
							<colgroup>
								<col width="4%">
								<col width="40%">
					...
					<div>
						...
						<div class="tabCont2">
							<div id="OrganArea">
								<div class="Dept_Tree" style="float: left; width: 60%; height: 212px; overflow-y: auto; zoom: 1; border: 1px solid #d2d2d2; background-color: rgb(245, 245, 245); margin: 0px;">
									<ul id="znTreeDept"></ul>
								</div>
								<div class="info_tb" style="float: right; width: 39%; overflow-y: auto; height: 212px;">
									<table class="pop_tbl">
										<caption></caption>
										<colgroup>
											<col width="10%">
							...
							<div id="GroupArea" style="display: none;">
								<div class="Group_Tree" style="float: left; width: 60%; height: 185px; overflow-y: auto; zoom: 1; border: 1px solid #d2d2d2; background-color: rgb(245, 245, 245); margin: 0px;">
